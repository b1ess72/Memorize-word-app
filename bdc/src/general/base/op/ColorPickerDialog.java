	...
	private class ColorPickerView extends View{
		...
		public ColorPickerView(Context context,int height,int width){
			...
			protected void onDraw(Canvas canvas){
				canvas.drawText("请点击中间的圆确认颜色",10, 20,mCenterPaint);
				...
				canvas.drawLine(rectLeft-offset*2,rectTop-offset,
						rectLeft+offset*2,rectTop-offset,mLinePaint);
			...
			public boolean onTouchEvent(MotionEvent event){
				...
				case MotionEvent.ACTION_MOVE:
					if(downInCircle&&inCircle){
						...
						mCenterPaint.setColor(interpCircleColor(mCircleColors,
								unit));
					...
					if(debug)Log.v(TAG,"[MOVE]高亮："+highlightCenter+"微亮："+highlightCenterLittle
							+"中心："+inCenter);
					if((highlightCenter&&inCenter)||(highlightCenterLittle&&
							inCenter)){
